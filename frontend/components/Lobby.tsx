...
interface Props {
  ...
  userCredits: number;
  onStart: (plan: InterviewPlan, sessionId: string, credits: number) => void;
  onBack: () => void;
}
...
const Lobby: React.FC<Props> = ({ config, userCredits, onStart, onBack }) => {
  ...
  const handleEnter = async () => {
    ...
    setError(null);
    try {
      const effectiveConfig = { ...config, duration: clampDuration(config.duration, config.plan) };
      const res = await BackendApi.startSession(effectiveConfig);
      const planRes = await BackendApi.generatePlan(res.sessionId);
      onStart(planRes.plan, planRes.sessionId, planRes.credits);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12">
      <header className="flex justify-between items-start">
        <div>
          <h2 className="text-4xl font-black tracking-tighter text-white">Pronto para o<br/><span className="text-indigo-500">Próximo Nível?</span></h2>
        ...
        <div className="absolute top-6 left-6 right-6 flex justify-between">
           ...
           <div className="native-glass px-4 py-2 rounded-2xl flex items-center gap-3">
              <div className="flex gap-0.5 items-center">
                 {[1,2,3].map(i => (
                   <div key={i} className={`w-1 rounded-full ${audioLevel > i * 10 ? 'bg-indigo-400' : 'bg-slate-700'}`} style={{height: '8px'}} />
                 ))}
              </div>
      ...
      {!hasCredits && (
        <div className="bg-red-500/10 border border-red-500/20 p-6 rounded-[2rem] text-center space-y-2">
           ...
           <p className="text-slate-400 text-[10px] font-medium leading-relaxed">Você precisa de pelo menos 1 crédito para iniciar uma simulação.</p>
        </div>
      )}
